...
%> Mar. 12th, 2024
    ...
    plotCamera('Location', [0;0;0], 'Orientation', eye(3), 'Opacity', 0.3, 'Size', 0.1, 'Color', [0,0,1]); hold on;
    for ci = 1:size(GT_Poses, 3)
        R = GT_Poses(:,1:3,ci);
        C = Abs_Cam_Center_GT(:,ci);
        plotCamera('Location', C, 'Orientation', R, 'Opacity', 0.3, 'Size', 0.1, 'Color', [0,0,1]);
        hold on;
    end
    ...
    plotCamera('Location', [0;0;0], 'Orientation', eye(3), 'Opacity', 0.3, 'Size', 0.1, 'Color', [0,1,0]); hold on;
    for ci = 1:size(Estimated_Poses, 3)
        R = Estimated_Poses(:,1:3,ci);
        C = Abs_Cam_Center(:,ci);
        plotCamera('Location', C, 'Orientation', R, 'Opacity', 0.3, 'Size', 0.1, 'Color', [0, 1, 0]);
        hold on;
    end
